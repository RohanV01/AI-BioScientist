...
| `Databases/papers/` | Fallback-only full-text PDF folder for `download_paper` (`orchestrator/app/tools/literature_discovery.py`) — used only when no Experiment is in scope (standalone/test tool calls outside a live agent run). A real agent run always saves into that Experiment's own `Experiments/<id>/papers/` instead (see below). | `download_paper`'s Camofox acquisition (drives the real Sci-Hub UI, captures the browser's own download). Without `CAMOFOX_API_URL` set, downloads simply fail gracefully and no papers land here. |
| `Experiments/<experiment-id>/` | One folder per research Experiment (`orchestrator/app/models.py`'s `Experiment`, `orchestrator/app/experiment_context.py`) — `papers/` (downloaded PDFs), `papers.json` (a manifest of every DOI `discover_papers` surfaced in that experiment, its content-integrity check result, and dedup status for repeat downloads/reads), `findings/` (per-paper structured extraction from `read_paper`), `conclusion.md` (`/experiment conclude`'s synthesis over that experiment's findings). Auto-created the moment an experiment opens (`/experiment start` or the first message in a channel with none open) — every clone of this repo gets the same relative-path convention, not a personal folder. | The Camofox full-text pipeline above, scoped per-investigation instead of one shared pile. |
