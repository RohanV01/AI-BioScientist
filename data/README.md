...
| `Databases/papers/` | Full-text PDFs downloaded by `download_paper` (`orchestrator/app/tools/literature_discovery.py`), one file per DOI. Auto-created on first download — every clone of this repo gets the same relative path, not a personal folder. | `download_paper`'s Camofox acquisition (drives the real Sci-Hub UI, captures the browser's own download). Without `CAMOFOX_API_URL` set, downloads simply fail gracefully and no papers land here. |
