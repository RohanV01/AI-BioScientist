...
| `Databases/papers/` | Full-text PDFs downloaded by `download_paper` (`orchestrator/app/tools/literature_discovery.py`), one file per DOI. Auto-created on first download — every clone of this repo gets the same relative path, not a personal folder. | `download_paper`'s Sci-Doc-Hub/Camofox acquisition waterfall. Without `SCI_DOC_HUB_MCP_URL`/`CAMOFOX_API_URL` set, downloads simply fail gracefully and no papers land here. |
