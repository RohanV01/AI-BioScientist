...
- **Claude Code/Codex Runner — Plan → Execute → Synthesize:** invokes the agentic loop (via the Claude Agent SDK) with the *full* resolved tool roster available for every query. The system prompt requires the model to (1) state its methodology — which tools it intends to use and why — before acting, (2) execute against that plan, calling whatever tools the task actually needs, (3) synthesize a final grounded report. The stated methodology is itself posted to the user, not just internal reasoning — this is the "show which puzzle pieces get assembled" requirement.
  ...
  **Critical isolation requirement — found the hard way, in two separate incidents, and both are required:**
  1. **`setting_sources=[]`** (Phase 1 leak). `ClaudeAgentOptions(allowed_tools=[...])` alone does not sandbox a run — the SDK's default (`setting_sources=None`) still loads the host's `~/.claude/settings.json` and every personal MCP connector configured there. A real test run used the developer's own personal PubMed connector instead of the intended tool, silently.
  2. **`strict_mcp_config=True`** (Phase 3 leak, a *different* mechanism than #1). Even with `setting_sources=[]` set, a real test run adding the ChEMBL tool still silently used the developer's personal **account-linked** `claude_ai_ChEMBL` connector instead of the wired one — `setting_sources` only governs *file-based* settings (`~/.claude/settings.json`, project `.mcp.json`); it does not touch connectors tied to the authenticated Claude.ai session itself. `strict_mcp_config=True` is the flag that actually restricts a run to *only* what's passed in `mcp_servers` — confirmed fixing this specific leak via a direct test (`test_isolation.py`), not just inferred from the SDK's docstring.

  **Both flags are required together; neither alone is sufficient.** Without them, the tool roster is not actually enforced, which is both a correctness bug (wrong tool, wrong/unverifiable grounding) and a security one — a multi-tenant deployment would leak the operator's own personal account access into every agent run.
