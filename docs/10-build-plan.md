...
- [ ] Stand up Mattermost locally (docker-compose), confirm Bot Accounts + Outgoing Webhooks work with a trivial echo bot — proves the messaging-layer mechanics before any agent logic exists. *(In progress: Postgres container is up; Mattermost container is next.)*
