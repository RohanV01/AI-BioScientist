...
- [ ] **Gap 9 compliance boundary — revised:** full-text sourcing is allowed to use Sci-Hub as an explicit fallback source (see Discovery & Acquisition design above), not restricted to OA-only as originally scoped. The compliance requirement is now: every full-text response must **disclose which tier it came from** (OA / BYO-paywalled / Sci-Hub), never presenting a Sci-Hub-sourced PDF as if it were an OA one — a provenance-labeling requirement, not a source restriction. Document this explicitly before Phase 1 ships anything literature-facing.

**Exit criterion:** a message sent to a trivial Mattermost bot triggers a stub Orchestrator response, the Discovery & Acquisition design is documented (this section) and the Sci-Hub lookup step's cost is measured (done — ~50-60s/query), and the revised Gap 9 provenance-labeling requirement is written down somewhere Phase 1 will actually reference.
