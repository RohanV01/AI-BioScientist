...
- [x] Stand up Mattermost locally (docker-compose), confirm Bot Accounts work with a trivial echo bot — done 2026-08-15 via `scripts/bootstrap_mattermost.sh` (idempotent, reusable by anyone cloning the repo). Verified with real HTTP status checks (not the field-presence heuristic the first draft of the script wrongly relied on — see its own history/comments): admin created, team created, bot created, bot posted a real message (HTTP 201, `props.from_bot: true`). **Outgoing Webhooks not yet done** — needs a receiving endpoint, which doesn't exist until the Orchestrator Service skeleton below is scaffolded. Revisit once that exists.
