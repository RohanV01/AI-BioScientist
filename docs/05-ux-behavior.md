...
3. Execution proceeds against that stated plan. If it's fast (a single live-MCP query), the final report can follow within seconds. If it's slow (an RxDis pipeline run, several chained tool calls), the agent posts progress updates in the same thread as steps complete (FR-7) — never silence for more than a short interval without an update.
