...
This project has access to a large already-cataloged tool inventory (Section 1 of the research report) to make that solvable now, delegated via chat instead of five to seven separate UIs.
...
- **One master agent** whose tool roster grows to include PubMed (already live, Phase 1) and further domain tools added incrementally — not multiple separate agents. See `10-build-plan.md` Phase 2/3.
...
| FR-3 | The agent can trigger a multi-step drug-discovery pipeline run (e.g. target identification or repurposing for a named disease) as one step of a larger plan, and report status/results back into the channel. | P0 |
...
| FR-7 | Long-running tasks (a multi-step pipeline run can take minutes) post progress updates into the thread rather than going silent. | P1 |
...
- **Resilience of long-running agents:** a multi-step pipeline run failing partway through should not corrupt Mattermost's thread state or leave the user without an error message.
